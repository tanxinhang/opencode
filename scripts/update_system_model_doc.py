...
def add_chinese_appendix(doc) -> None:
    ...
    bullet(doc, "正式统计（5000 训练/5000 测试几何）：deflection 作为预测分时 Spearman 0.588，低于 0.6；改用精确 P_D 增益预测分后 Spearman 0.93、logit-P_D 预测分 0.91（300 帧 smoke），说明代理失败点在 deflection，P_D 增益选择器可通过 G1-A。")
    ...
    bullet(doc, "N=8/12、Q=3/5、B_max=20/40、5 种子的公平全局预算扫描：Proposed 平均 P_D 0.886，Sensing-SNR Top-K 0.886，Independent-Deflection Top-K 0.884，Communication Top-K 0.743，All-scheduled 0.933；")
    bullet(doc, "此前“Proposed 落后 3.6 pp”来自不公平的逐目标预算基线，已修正；")
    bullet(doc, "精确 P_D 增益贪心为 0.885，未超过 deflection 贪心；J 散度替代目标在异方差矩下与 P_D 不一致，已作为负结果记录；")
    bullet(doc, "强相关模型（最高 SNR 报告对 rho=0.85）下，条件贪心平均 P_D 0.869 高于 Independent-Deflection Top-K 0.859，在 77.5% 配置中胜出；精确 P_D 贪心 0.880 为最强变体，这是系统级相关穿透证据。")

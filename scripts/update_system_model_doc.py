...
def add_chinese_appendix(doc) -> None:
    ...
    bullet(doc, "同规模资源公平表（30 trials/列）：固定每帧能量时 L=4 将场景恢复从 86.7% 提到 100%，但感知能量为 4 倍；固定总能量（L=4, A=1）时降至 50%，说明增益来自资源而非时间分集。")
    bullet(doc, "时间-带宽账本：L=1 总占用 2575 符号（感知 512、身份 2048、报告 15），L=4 为 4111 符号；固定总时间-带宽路径需 OTFS 网格缩放，暂未运行。")
    ...
    bullet(doc, "非饱和压力 Gate（三目标、最强报告对 rho=0.9、中等 SNR）：B=6 时条件贪心平均 P_D 0.692 vs 静态 Top-K 0.520（+0.172，CI [0.161,0.181]，胜率 100%）；B=9 时 0.813 vs 0.699（+0.114，CI [0.105,0.123]）；最差目标 P_D 分别从 0.471 升至 0.569、0.577 升至 0.782；B=12 饱和后增益消失。")
    bullet(doc, "规模扫描（10 seeds，预算随 Q 线性）：Exact-P_D 相对 Static ID Top-K 的增益随 M/Q 增大，(M,Q)=(8,3) 为 +0.006、(12,5) 为 +0.007、(16,5) 为 +0.015、(16,8) 为 +0.067；最差目标在 (16,8) 提升 +0.127；Exact-P_D 在 (16,8) 耗时约 0.25 s，条件贪心约 0.15 s。")
    bullet(doc, "非饱和规模压力 Gate（20 seeds，4 报告/目标，预算 3 报告/目标）：条件贪心相对 Static ID Top-K 在 Q=3/5/8 时平均 P_D 均提升约 +0.114；最差目标改善从 Q=3 的 +0.205 增至 Q=8 的 +0.272，优势随规模持续可见。")

"""G48 gate: target-wise centralized/distributed architecture switch.

G47 selects one global mode for all targets.  G48 additionally allows each
target to choose its own better architecture.  The target-wise policy keeps
the exact per-target P_D of the centralized soft schedule and the peer
majority branch, selects ``max(soft_q, peer_q)`` for each target, and uses
the following order inequality as a certificate:

``min_q max(a_q, b_q) >= max(min_q a_q, min_q b_q)``.

The selected soft targets use no more report bits than the centralized
schedule already computed; peer targets spend zero report bits.
"""
...
from uav_otfs_isac.architecture_switch import target_wise_architecture_switch
    ...
    *, output: Path, seeds: int, budgets, grid: int, qos_target: float,
    ...
    for total_budget in budgets:
        ...
        for seed in seed_list:
            ...
            modes, values = target_wise_architecture_switch(
                soft_pds, peer_pds
            )
            ...
            target_wise_worst.append(float(np.min(values)))
            soft_used.append(int(soft.used_bits))
            peer_used.append(0)
            peer_target_rate.append(float(
                sum(mode == "peer" for mode in modes) / len(modes)
            ))
        ...
        summary.append({
            ...
            "target_wise_gain_vs_soft": target_mean - soft_mean,
            "target_wise_gain_vs_global": target_mean - global_mean,
            "peer_target_selection_rate": float(np.mean(peer_target_rate)),
            "soft_rho_exact": effective_deflection(
                soft_mean, false_alarm_rate
            ) / max(full_mean, 1e-12),
            "target_wise_rho_exact": effective_deflection(
                target_mean, false_alarm_rate
            ) / max(full_mean, 1e-12),
            "soft_used_bits": float(np.mean(soft_used)),
            "peer_used_bits": float(np.mean(peer_used)),
            "soft_qos_feasible": soft_mean >= qos_target - 1e-9,
            "target_wise_qos_feasible": target_mean >= qos_target - 1e-9,
        })

    payload = {
        "gate": "G48-target-wise-architecture-switch",
        "seeds": seeds,
...
def main() -> None:
    ...
    parser.add_argument("--output", default="results/target_wise_architecture_switch_gate.json")

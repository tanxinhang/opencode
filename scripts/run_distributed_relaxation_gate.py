"""G39 gate: distributed features under lower budgets and QoS thresholds.

The centralized performance is close to saturation, so this gate lowers the
total budget and the QoS target, and increases distributed features: peer
majority, multi-hop degraded consensus, and optimized hard decisions.
"""
...
def run_gate(
    *, output: Path, seeds: int, budgets, qos_targets, grid: int,
    ...
    for total_budget in budgets:
        ...
        for qos_target in qos_targets:
            ...
            for seed in seed_list:
                ...
                for model in models:
                    reports = sorted(
                        set(range(model.num_uavs)) - {model.owner}
                    )
    ...
    payload = {
        "gate": "G39-distributed-relaxation",
        "seeds": seeds,
        "summary": summary,
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", default="results/distributed_relaxation_gate.json")
    parser.add_argument("--seeds", type=int, default=2)
    parser.add_argument("--budgets", type=int, nargs="+", default=[20, 24, 28])
    parser.add_argument("--qos-targets", type=float, nargs="+",

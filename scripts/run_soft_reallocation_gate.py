"""G49 gate: soft-report reallocation after target-wise switching.

G48 selects per-target modes and leaves the centralized soft schedule fixed.
When a target switches to peer consensus, its previously scheduled soft
reports are freed.  G49 adds those freed bits back to the remaining
centralized targets with an exact expected-P_D greedy step.  The update is
additive, so per-target centralized P_D is nondecreasing and the target-wise
worst P_D cannot decrease.
"""
    ...
    *, output: Path, seeds: int, budgets, grid: int, qos_target: float,
    ...
    for total_budget in budgets:
        ...
        for seed in seed_list:
            ...
            modes, target_values = target_wise_architecture_switch(
                soft_pds, peer_pds
            )
            ...
            target_wise_worst.append(float(np.min(target_values)))
            target_wise_used.append(sum(
                int(models[q].report_bits[uav])
                for q in range(len(models))
                if modes[q] == "soft"
                for uav in soft.scheduled[q]
                if uav != models[q].owner
            ))

            realloc_schedules, realloc_quality, realloc_bits = (
                reallocate_soft_report_bits(
                    models, modes, soft.scheduled, report_budget,
                    false_alarm_rate, grid=grid,
                )
            )
            realloc_values = [
                realloc_quality[q] if modes[q] == "soft" else peer_pds[q]
                for q in range(len(models))
            ]
            realloc_worst.append(float(np.min(realloc_values)))
            realloc_used.append(int(realloc_bits))
        ...
        realloc_mean = float(np.mean(realloc_worst))
        summary.append({
            ...
            "reallocation_worst_pd": realloc_mean,
            "reallocation_gain_vs_soft": realloc_mean - soft_mean,
            "reallocation_gain_vs_global": realloc_mean - global_mean,
            "reallocation_gain_vs_target_wise": realloc_mean - target_mean,
            "soft_rho_exact": effective_deflection(
                soft_mean, false_alarm_rate
            ) / max(full_mean, 1e-12),
            "reallocation_rho_exact": effective_deflection(
                realloc_mean, false_alarm_rate
            ) / max(full_mean, 1e-12),
            "target_wise_used_bits": float(np.mean(target_wise_used)),
            "reallocation_used_bits": float(np.mean(realloc_used)),
            "soft_qos_feasible": soft_mean >= qos_target - 1e-9,
            "reallocation_qos_feasible": realloc_mean >= qos_target - 1e-9,
        })

    payload = {
        "gate": "G49-soft-report-reallocation",
        "seeds": seeds,
...
def main() -> None:
    ...
    parser.add_argument("--output", default="results/soft_reallocation_gate.json")

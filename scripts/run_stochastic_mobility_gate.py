"""G51 gate: stochastic mobility with RIS reconfiguration latency.

G26 used deterministic trajectories.  G51 increases the system-model
difficulty by drawing AR(1)-correlated random UAV and target positions, a
random time-varying blockage, and by comparing RIS designs that are
reconfigured every frame, designed from the previous frame (one-frame
latency), or frozen from the initial frame.

The target-wise architecture switch (G48) and the two-sided mode ascent
(G50) run independently on every frame.  Since the mode ascent is monotone
per frame, the worst-over-time P_D is also nondecreasing relative to the
target-wise baseline.  No claim is made that stochastic samples estimate a
continuous-time process without the declared frame model.
"""
...
from uav_otfs_isac.stochastic_mobility import stochastic_trajectories
    ...
    *, output: Path, seeds: int, frames: int, grid: int,
    ...
    for seed in seed_list:
        ...
        for time_index in range(frames):
            ...
            no_ris_soft, no_ris_target, no_ris_ascent = (
                target_wise_and_ascent_pd(
                    no_ris_models, report_budget, false_alarm_rate,
                    qos_pd, qos_weights, grid,
                )
            )
            methods["no_ris_soft"].append(no_ris_soft)
            methods["no_ris_mode_ascent"].append(no_ris_ascent)
    ...
    summary["latency_over_static_worst_gain"] = (
        summary["methods"]["ris_latency_mode_ascent"]["worst_over_time"]
        - summary["methods"]["ris_static_mode_ascent"]["worst_over_time"]
    )
    summary["ideal_ascent_over_target_wise_worst_gain"] = (
        summary["methods"]["ris_ideal_mode_ascent"]["worst_over_time"]
        - summary["methods"]["ris_ideal_target_wise"]["worst_over_time"]
    )
    summary["ideal_ascent_qos_gain"] = (
        summary["methods"]["ris_ideal_mode_ascent"]["qos_over_time"]
        - summary["methods"]["ris_ideal_target_wise"]["qos_over_time"]
    )

    payload = {
        "gate": "G51-stochastic-mobility",
        "seeds": seeds,
        "frames": frames,
        "summary": summary,
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", default="results/stochastic_mobility_gate.json")

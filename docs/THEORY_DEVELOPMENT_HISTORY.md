...
- 结果(**registered, clean-HEAD, sign-aware audit**): 冻结操作点 worst-target 延迟(held-out 12k/arm,共享 CRN blocks,paired per-block bootstrap 95% CI):`A 5.059 → B00 4.288(Δ_owner +15.2%,CI[0.716,0.825])→ B0 3.138(Δ_π +26.8%,CI[1.089,1.216],**主导,占 A→C 总收益 84%**)→ B1 3.747(Δ_λ −19.5%,CI[−0.682,−0.537]:**λ 使最坏目标时延变差**)→ C 3.690(Δ_admission +1.5%,CI[0.037,0.077])。Δ_owner 是真实的正贡献(15%),但**最大正向机制是 detection-deficit task price(Δ_π)**,λ 在本 cell 是"能力/可行性 steer"而非时延机制(delay 为负),density 为内务细节。QoS 全臂 frozen 点 UNCERTAIN(预期;anytime/matched-QoS 证书属 P4.2/P4.2b)。
- 审计(advice/017 §十三 #audit-1-3): (1) dominant 判定改为**最大正向 Δ**(`is_gain=True`),负 Δ(如 λ)永不许选为 dominant,无正向时给出 "no dominant positive mechanism" 兜底;(2) Δ 措辞 sign-aware(亏损读 "J_cur > J_prev by |d| (HURTS)",不再出现自相矛盾的 "J_cur < J_prev by -x");(3) 语义陷阱明示: B00 = owner-direct 证据面 + **移除 v2 的 local-deficit 价**(非纯路由),若 Δ_owner 主导须措辞为 "owner-directed architecture 连同 deficit 价移除",不得声称纯路由。
- 影响: 论文机制叙事——**任务价(detection-deficit coordination)为一级算法贡献、owner-directed 架构为一级架构贡献、λ 为可行性 steer(不得声称降延迟)、density 为内务**;P5-A registered(provenance git_dirty=false, git_commit=6291cdc, `results/p5a_ablation_ladder.json`)。下一步 P5-A 第二优先级 **cross-geometry**(G=20~30 独立几何、每几何 500~2000 runs,测 `Pr_G(J_ca<J_v2)` 与 `Pr_G(CA QoS feasible)`)与 (K,Q,ρ) scaling,然后 P5-B control bus 物理化。
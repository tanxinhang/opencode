...
| 4 | Ablations mixed | DONE: 500-seed factorial ablation toggles each factor one at a time; a hard weak-target scenario shows max-min gains of 2.45-3.45 pp; a quantization study adds a water-filling-inspired greedy plus an exact joint oracle; a strong-vs-weak multi-target gate shows 3.61-4.95 pp gains; Q=2..20 sweeps at R=4/5 keep gains at 1.5-2.7 pp, and vectorized R=8 at a 7Q budget keeps gains at 1.60-3.84 pp; a MAPPO baseline underperforms exact joint by 6.4-7.1 pp; a BSC difficulty audit shows gains vanish at flip >=0.05, so the mild channel is kept deliberately. |
| 5 | Hard B&B instances missing | DONE: `scripts/run_scaled_difficulty_gate.py` adds critical-threshold, similar-weak, K-report, and correlated-redundant layers with node counts, recursion depth, prune rate, and exhaustive match checks. A dual Cauchy bound tightens the old bound and cuts the correlated-redundant tree from 1791 to 113 nodes. Proposition 1 adds a Pareto-frontier bound and complexity `O(Q |F| log K)`. Joint option generation is vectorized to R=8, and a branch-and-bound threshold solver reaches R=12 (0.2-2.9 s per query). |

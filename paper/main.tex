...
A third arm applies discrete water-filling to the bit allocation: under high-resolution scalar quantization the distortion of a $b$-bit quantizer scales as $2^{-2b}$, so the marginal evidence gain of each additional bit is decreasing.  Starting from one bit per report, the greedy rule gives the next bit to the report with the largest marginal per-report $P_D$ gain; for a separable, diminishing-returns allocation this is the discrete KKT / water-filling solution.  At $B=18$ it exceeds the fixed 1-4 pattern by 1.61 pp (0.926 vs 0.910) and at $B=24$ it remains better than the pattern by 0.85 pp (0.941 vs 0.932), while fixed 3-bit reporting is still the best arm once its full 24-bit budget is available.
\begin{table}[htbp]
\centering
\resizebox{\linewidth}{!}{
